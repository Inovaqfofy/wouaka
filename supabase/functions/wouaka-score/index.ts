...
function calculateScore(input: ScoreRequest): ScoreResult {
  ...
  finalScore = Math.max(0, finalScore - fraudAnalysis.fraud_score * 0.4);
  
  // Data quality
  const dataQuality = avgConfidence >= 70 ? 'high' : avgConfidence >= 50 ? 'medium' : avgConfidence >= 30 ? 'low' : 'insufficient';
