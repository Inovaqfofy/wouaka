...
export const useSubscription = () => {
  ...
  const { data: plans = [], isLoading: plansLoading } = useQuery({
    ...
    queryFn: async () => {
      ...
      return (data || []).map(plan => ({
        ...
        limits: plan.limits as { scores_per_month: number; api_calls_per_month: number } || { scores_per_month: 0, api_calls_per_month: 0 }
      })) as SubscriptionPlan[];
    },
  });
      ...
      if (data && data.plan) {
        ...
        const subscription: Subscription = {
          ...
          plan: {
            ...
            limits: (planData.limits as { scores_per_month: number; api_calls_per_month: number }) || { scores_per_month: 0, api_calls_per_month: 0 },
  ...
  // Fetch usage (scoring requests count for current month)
  const { data: usage = { scoresUsed: 0 } } = useQuery({
    queryKey: ['subscription-usage', user?.id],
    queryFn: async () => {
      if (!user?.id) return { scoresUsed: 0 };
