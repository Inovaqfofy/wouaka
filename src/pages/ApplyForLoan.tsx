...
const ApplyForLoan = () => {
  ...
  const processScoring = async () => {
    ...
    try {
      ...
      // Call W-Score edge function
      const { data: scoreResult, error: scoreError } = await supabase.functions.invoke('wouaka-score', {
        body: {
          phone_number: form.getValues('applicant_phone'),
          full_name: form.getValues('applicant_name'),
          national_id: form.getValues('national_id'),
          data_sources: ['telecom', 'mobile_money'],
        }
      });
      ...
      await updateApplicationResults.mutateAsync({
        ...
        risk_level: scoreResult?.risk_level || 'high',
        kyc_status: 'verified',
