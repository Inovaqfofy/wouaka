...
const SubmitDocuments = () => {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      // Trigger W-KYC processing via edge function
      try {
        const { data: kycResult, error: kycProcessError } = await supabase.functions.invoke('wouaka-kyc', {
          body: {
            level: 'basic',
            full_name: tokenData.client_name,
            phone_number: tokenData.client_phone || '',
            national_id: extractedText.slice(0, 50), // Use extracted text as ID proxy
            date_of_birth: '1990-01-01', // Placeholder - would be extracted from OCR
            country: 'CI',
            ocr_data: {
              national_id: nationalIdDoc?.ocrData || null,
        ...
        if (!kycProcessError && kycResult) {
          ...
          await supabase
            ...
            .update({
              ...
              identity_score: kycResult.risk_score || null,
              fraud_score: 100 - (kycResult.risk_score || 50),
